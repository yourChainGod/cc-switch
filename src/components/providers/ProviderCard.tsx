...
function ProviderCardImpl({
  ...
  const failoverActionLabel = isInFailoverQueue
    ? t("failover.removeQueue", {
        defaultValue: "移出故障转移队列",
      })
    : t("failover.addQueueFull", {
        defaultValue: "加入故障转移队列",
      });
        ...
        <div className="flex flex-1 items-center gap-2">
          ...
          <div className="space-y-1">
            <div className="flex flex-wrap items-center gap-2 min-h-7">
              ...
              {isInFailoverQueue && failoverPriority && (
              ...
              {isInFailoverQueue && (
                <span
                  className={cn(
                    ...
                    isFailoverQueueEffective
                      ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-300"
                      : "bg-amber-500/10 text-amber-700 dark:text-amber-300",
                  )}
                  title={
                    isFailoverQueueEffective
                      ? t("failover.status.activeTooltip", {
                          defaultValue:
                            "自动故障转移已生效，该供应商会按队列优先级参与路由。",
                        })
                      : t("failover.status.pendingTooltip", {
                          defaultValue:
                            "已加入队列，但需要同时启用路由接管、启动路由服务并开启自动故障转移后才会生效。",
                        })
                  }
                >
                  {isFailoverQueueEffective
                    ? t("failover.status.active", {
                        defaultValue: "生效中",
                      })
                    : t("failover.status.pending", {
                        defaultValue: "待生效",
                      })}
                </span>
              )}
              ...
              {hasKeyPool && keySummary && (
                <span
                  ...
                  title={t("providerKeys.summaryTooltip", {
                    total: keySummary.total,
                    available: keySummary.available,
                    degraded: keySummary.degraded,
                    cooldown: keySummary.cooldown,
                    disabled: keySummary.disabled,
                    priority: keySummary.minPriority ?? "-",
                    defaultValue:
        ...
        <div className="flex items-center ml-auto min-w-0 gap-3">
          ...
          {canUseFailoverQueue && (
            <Button
              type="button"
              size="sm"
              variant={isInFailoverQueue ? "secondary" : "outline"}
              onClick={() => onToggleFailover?.(!isInFailoverQueue)}
              aria-label={failoverActionLabel}
              title={failoverActionLabel}
              className={cn(
                "shrink-0 px-2.5",
                isInFailoverQueue
                  ? "border-blue-500/30 bg-blue-500/10 text-blue-700 hover:bg-blue-500/15 dark:text-blue-300"
                  : "border-border-default text-muted-foreground hover:text-blue-700 dark:hover:text-blue-300",
              )}
            >
              <Route className="h-4 w-4" />
              <span className="hidden sm:inline">
                {isInFailoverQueue
                  ? t("failover.inQueue", { defaultValue: "队列中" })
