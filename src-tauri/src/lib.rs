...
fn show_main_window(app: &tauri::AppHandle) -> Result<WebviewWindow, String> {
    ...
    log::debug!(
        "主窗口状态: visible={:?}, minimized={:?}, size={:?}, position={:?}",
        window.is_visible(),
        window.is_minimized(),
        window.outer_size(),
        ...
        Ok(request) => {
            ...
            if focus_main_window {
                if show_main_window(app).is_ok() {
                    ...
                    log::error!("✗ Failed to show and focus window");
                }
            }
        }
        Err(e) => {
...
pub fn run() {
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            if let Some(window) = app.get_webview_window("main") {
                ...
                if settings.silent_startup {
                    ...
                    // 正常启动模式：显示窗口
                    match show_main_window(app.handle()) {
                        Ok(_) => log::info!("正常启动模式：主窗口已显示"),
                        Err(e) => log::error!("正常启动模式：主窗口显示失败: {e}"),
                    }
                }
            } else if !settings.silent_startup {
                match show_main_window(app.handle()) {
                    Ok(_) => log::info!("正常启动模式：主窗口已重建并显示"),
                    Err(e) => log::error!("正常启动模式：主窗口重建失败: {e}"),
                }
            }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::get_model_routing_config,
            commands::set_model_routing_config,
        ...
        #[cfg(target_os = "macos")]
        {
            match event {
                ...
                RunEvent::Reopen { .. } => {
                    ...
                    } else if let Err(e) = show_main_window(app_handle) {
                        log::error!("恢复主窗口失败: {e}");
                    }
                }
