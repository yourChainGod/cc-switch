...
pub fn transform_claude_request_for_api_format(
    ...
    let session_cache_key: Option<String> = session_id
        .map(str::trim)
        .filter(|s| !s.is_empty())
    ...
    match api_format {
        "openai_responses" => {
            log::debug!(
                "[Cache] OpenAI Responses prompt_cache_key source={cache_key_source}, provider={}, has_key={}",
                provider.id,
                cache_key.is_some()
            );
            super::transform_responses::anthropic_to_responses(body, cache_key)
        }
        "openai_chat" => {
