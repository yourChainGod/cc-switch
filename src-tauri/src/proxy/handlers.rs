...
async fn handle_claude_transform(
    ...
    let use_streaming = should_use_claude_transform_streaming(is_stream, response.is_sse());
...
fn should_use_claude_transform_streaming(requested_streaming: bool, upstream_is_sse: bool) -> bool {
    requested_streaming || upstream_is_sse
}
...
fn responses_sse_to_response_value(body: &str) -> Result<Value, ProxyError> {
    ...
    // 已聚合好的完整响应（错标 SSE 兜底路径复用本函数）
    let mut process_block = |block: &str, strict: bool| -> Result<(), ProxyError> {
        // 残余尾块（strict=false）在已拿到 completed 后整体跳过——已完成后再执行
        // 残余里的完整 response.failed/杂事件会把成功响应翻成 422（C8）。
...
data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_tail\",\"status\":\"completed\",\"model\":\"gpt-5.4\",\"output\":[],\"usage\":{\"input_tokens\":3,\"output_tokens\":1}}}\n";
    ...
    fn responses_sse_to_response_value_ignores_truncated_trailing_block() {
        // 截断的残余尾块不能破坏已聚合好的完整响应（错标 SSE 兜底路径复用本函数）
        let sse = "event: response.completed\n\
