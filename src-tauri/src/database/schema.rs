...
impl Database {
    ...
    pub(crate) fn create_tables_on_conn(conn: &Connection) -> Result<(), AppError> {
        ...
        conn.execute("CREATE INDEX IF NOT EXISTS idx_request_logs_provider ON proxy_request_logs(provider_id, app_type)", [])
            .map_err(|e| AppError::Database(e.to_string()))?;
        conn.execute("CREATE INDEX IF NOT EXISTS idx_request_logs_provider_key ON proxy_request_logs(provider_key_id)", [])
            .map_err(|e| AppError::Database(e.to_string()))?;
        ...
        let result = (|| {
            while version < SCHEMA_VERSION {
                match version {
                    ...
                    10 => {
                        log::info!("迁移数据库从 v10 到 v11（添加 Provider Key Pool）");
                        Self::migrate_v10_to_v11(conn)?;
                        Self::set_user_version(conn, 11)?;
                    }
                    11 => {
                        log::info!("迁移数据库从 v11 到 v12（添加网关站点账号管理）");
    ...
    /// v10 -> v11 迁移：添加 Provider Key Pool
    fn migrate_v10_to_v11(conn: &Connection) -> Result<(), AppError> {
        Self::create_provider_keys_table(conn)?;
        Self::create_session_affinity_table(conn)?;
        Self::create_working_channel_affinity_table(conn)?;
        log::info!("v10 -> v11 迁移完成：已添加 provider_keys/session_affinity 表");
        Ok(())
    }

    /// v11 -> v12 迁移：历史上添加网关站点账号表；功能已移除，仅保留版本步进。
    /// 旧库中已存在的 gateway_* 表不再被任何代码读写，保留不动。
    fn migrate_v11_to_v12(_conn: &Connection) -> Result<(), AppError> {
        log::info!("v11 -> v12 迁移完成：网关站点账号功能已移除，跳过建表");
        Ok(())
    }

    /// v12 -> v13 迁移：历史上为站点账号添加签到字段；功能已移除，仅保留版本步进。
    fn migrate_v12_to_v13(_conn: &Connection) -> Result<(), AppError> {
        log::info!("v12 -> v13 迁移完成：网关签到功能已移除，跳过加列");
        Ok(())
    }
    ...
    fn create_provider_keys_table(conn: &Connection) -> Result<(), AppError> {
        ...
        .map_err(|e| AppError::Database(format!("创建 provider_keys 表失败: {e}")))?;

        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_provider_keys_provider
             ON provider_keys(app_type, provider_id, enabled, priority)",
            [],
        )
        .map_err(|e| AppError::Database(format!("创建 provider_keys provider 索引失败: {e}")))?;

        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_provider_keys_status
             ON provider_keys(app_type, provider_id, status, cooldown_until)",
            [],
        )
    ...
    fn create_session_affinity_table(conn: &Connection) -> Result<(), AppError> {
        conn.execute(
        ...
        .map_err(|e| AppError::Database(format!("创建 session_affinity 表失败: {e}")))?;

        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_session_affinity_expires
             ON session_affinity(expires_at)",
            [],
        )
        .map_err(|e| AppError::Database(format!("创建 session_affinity expires 索引失败: {e}")))?;

        Ok(())
    }

    fn create_working_channel_affinity_table(conn: &Connection) -> Result<(), AppError> {
        conn.execute(
            "CREATE TABLE IF NOT EXISTS working_channel_affinity (
        ...
        .map_err(|e| AppError::Database(format!("创建 working_channel_affinity 表失败: {e}")))?;

        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_working_channel_affinity_expires
             ON working_channel_affinity(expires_at)",
            [],
        )
        .map_err(|e| {
            AppError::Database(format!(
                "创建 working_channel_affinity expires 索引失败: {e}"
            ))
        })?;
