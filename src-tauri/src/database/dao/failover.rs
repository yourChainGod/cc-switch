...
impl Database {
    ...
    /// 智能默认故障转移顺序（ccLoad/cch 风格：启用即参与）。
    ///
    /// 未手动维护队列时，所有支持路由的供应商按首页排序自动参与
    /// 故障转移；官方登录类供应商（category = 'official'）不走本地路由，排除。
    pub fn get_default_failover_order(&self, app_type: &str) -> Result<Vec<String>, AppError> {
        let conn = lock_conn!(self.conn);

        let mut stmt = conn
            .prepare(
                "SELECT id FROM providers
                 WHERE app_type = ?1 AND COALESCE(category, '') != 'official'
                 ORDER BY COALESCE(sort_index, 999999), id ASC",
            )
            .map_err(|e| AppError::Database(e.to_string()))?;
