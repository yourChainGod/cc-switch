...
use crate::provider::{
    apply_provider_key_to_config, clear_provider_key_value, extract_provider_config_key, Provider,
    ProviderKey, ProviderKeyInput, UsageResult,
};
...
const CONFIG_KEY_MODE_MANUAL: &str = "manual";
    ...
    async fn update_current_claude_provider_syncs_live_when_proxy_takeover_detected_without_backup()
    {
        ...
        assert_eq!(
            live.get("env")
                ...
                .and_then(|v| v.as_str()),
            Some("model-updated"),
            "model override should follow the edited provider verbatim in takeover live config"
        );
    }
...
impl ProviderService {
    ...
    fn sync_config_key_pool_from_provider(
        state: &AppState,
        app_type: &AppType,
        provider: &mut Provider,
    ) -> Result<(), AppError> {
        let Some(config_key) = extract_provider_config_key(app_type.as_str(), provider) else {
            return Ok(());
        };
        ...
        let config_key_id = if let Some(existing) = existing {
            if !existing.enabled
                || matches!(
                    existing.status,
                    crate::provider::ProviderKeyStatus::Disabled
                )
            {
                let priority = existing.priority;
                let weight = existing.weight.max(1);
                ...
                let _ = state.db.update_provider_key(
                    ...
                    &ProviderKeyInput {
                        name,
                        key_value: existing.key_value.clone(),
                        auth_field: Some(config_key.auth_field.clone()),
                        enabled: true,
                        priority,
                        weight,
                        usage_script: None,
            ...
            let first_priority = keys
                ...
                .min()
                .map(|priority| priority.saturating_sub(10))
                .unwrap_or(0);
            let key = state.db.add_provider_key(
                app_type.as_str(),
                provider.id.as_str(),
                &ProviderKeyInput {
    ...
    fn config_key_mode(provider: &Provider) -> Option<&str> {
        provider
            .meta
            .as_ref()
    ...
    fn is_auto_config_key_provider(provider: &Provider) -> bool {
        match Self::config_key_mode(provider) {
            Some(CONFIG_KEY_MODE_AUTO) => true,
            Some(CONFIG_KEY_MODE_MANUAL) => false,
            // Legacy providers that already store configKeyId were effectively manual:
            // keep that choice until the selected row becomes unavailable.
            _ => provider
                .meta
                .as_ref()
    ...
    fn set_config_key_with_mode(
        ...
        let key = state
            ...
            .ok_or_else(|| {
                AppError::Message(format!(
                    "Provider key '{}' does not exist for provider '{}'",
                    key_id, provider_id
                ))
            })?;
        if !key.enabled || matches!(key.status, crate::provider::ProviderKeyStatus::Disabled) {
            return Err(AppError::Message(
                "Cannot use a disabled provider key as the direct config key".to_string(),
            ));
        }
        ...
        state
            ...
            .ok_or_else(|| {
                AppError::Message(format!(
                    "Provider '{}' disappeared after config key update",
                    provider_id
                ))
            })
    }
    ...
    fn sync_auto_config_key_binding(
        ...
        if !Self::is_auto_config_key_provider(&provider) {
            return Ok(());
        }

        if let Some(best_key) = Self::best_available_config_key(state, &app_type, provider_id)? {
            let direct_config_matches = extract_provider_config_key(app_type.as_str(), &provider)
                .is_some_and(|config_key| {
                    let field_matches = best_key
                        .auth_field
                        .as_deref()
                        .map(str::trim)
                        .filter(|field| !field.is_empty())
                        .is_none_or(|field| field == config_key.auth_field);
                    field_matches && config_key.key_value == best_key.key_value
                });
            let already_bound = provider.meta.as_ref().is_some_and(|meta| {
                meta.config_key_id.as_deref() == Some(best_key.id.as_str())
                    && meta.config_key_mode.as_deref() == Some(CONFIG_KEY_MODE_AUTO)
            }) && direct_config_matches;
            if !already_bound {
                Self::set_config_key_with_mode(
                    state,
                    app_type,
                    provider_id,
                    &best_key.id,
                    CONFIG_KEY_MODE_AUTO,
                )?;
            }
            return Ok(());
        }

        Self::clear_config_key_binding(state, app_type, provider)?;
        Ok(())
    }

    fn clear_config_key_binding(
        state: &AppState,
        app_type: AppType,
        provider: Provider,
    ) -> Result<(), AppError> {
    ...
    /// Add a key-pool entry and keep auto direct config selection on the best available key.
    pub fn add_key(
        state: &AppState,
        app_type: AppType,
        provider_id: &str,
        ...
        let key = state
            .db
            .add_provider_key(app_type.as_str(), provider_id, input)?;

        if key.enabled {
            Self::sync_auto_config_key_binding(state, app_type, provider_id)?;
        }

        Ok(key)
    }

    /// Use one key-pool entry as the provider's direct/local-app API key.
    pub fn set_config_key(
        ...
        Self::set_config_key_with_mode(state, app_type, provider_id, key_id, CONFIG_KEY_MODE_MANUAL)
    }

    /// Let the provider's direct/local-app API key follow the best available key-pool entry.
    pub fn set_config_key_auto(
        state: &AppState,
        app_type: AppType,
        provider_id: &str,
    ) -> Result<Provider, AppError> {
        let Some(best_key) = Self::best_available_config_key(state, &app_type, provider_id)? else {
            return Err(AppError::Message(
                "No available provider keys can be used as the direct config key".to_string(),
            ));
        };
    ...
    /// Keep a provider's direct config key valid after a key-pool row changes.
    pub fn repair_config_key_binding(
        state: &AppState,
        app_type: AppType,
        provider_id: &str,
        changed_key_id: &str,
        ...
        let current_config_key_id = provider
            .meta
            .as_ref()
            .and_then(|meta| meta.config_key_id.as_deref());
        if current_config_key_id != Some(changed_key_id) {
            return Ok(());
        }
        ...
        if let Some(key) =
            state
                .db
                .get_provider_key(app_type.as_str(), provider_id, changed_key_id)?
        {
            if key.enabled && !matches!(key.status, crate::provider::ProviderKeyStatus::Disabled) {
                Self::set_config_key_with_mode(
                    state,
        ...
        if let Some(next_key) = Self::best_available_config_key(state, &app_type, provider_id)? {
            Self::set_config_key_with_mode(
                state,
                app_type,
                provider_id,
