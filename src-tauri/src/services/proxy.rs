...
/// 代理接管模式下需要从 Claude Live 配置中同步的"模型覆盖"字段。
///
/// 接管时这些字段与"直接启动"该供应商保持一致：先清除 live 中可能残留的
/// 上一个供应商的值，再从目标供应商配置原样快照（含 `[1M]` 等能力标记）。
/// 客户端拿到的模型与直连完全相同，1M 上下文等能力由 Claude Code 自身的
/// 协议头协商；代理转发侧只负责剥离上游不认识的本地标记。
...
impl ProxyService {
    ...
    fn apply_claude_takeover_fields(config: &mut Value, proxy_url: &str) {
        // 先 snapshot 再 remove/insert，保证模型字段原样保留。
        let takeover_model_fields = Self::build_claude_takeover_model_fields(config);

        Self::apply_claude_takeover_fields_with_models(config, proxy_url, takeover_model_fields);
    }

    fn apply_claude_takeover_fields_with_models(
        config: &mut Value,
        proxy_url: &str,
    ...
    /// 原样快照模型相关字段：客户端拿到的模型（含 `[1M]` 等能力标记）
    /// 与直连该供应商时完全一致，不做任何别名改写。
    fn build_claude_takeover_model_fields(config: &Value) -> Vec<(&'static str, String)> {
        ...
        CLAUDE_MODEL_OVERRIDE_ENV_KEYS
            .iter()
            .filter_map(|key| {
                Self::claude_env_string(env, key).map(|value| (*key, value.to_string()))
            })
            .collect()
    }
    ...
    async fn takeover_live_configs(&self) -> Result<(), String> {
        ...
        if let Ok(mut live_config) = self.read_claude_live() {
            // 校验当前供应商存在；不存在则整体接管失败
            self.require_current_provider_for_app(&AppType::Claude)?;
    ...
    async fn hot_switch_provider_updates_claude_live_while_preserving_takeover_fields() {
        ...
        assert_eq!(
            live.get("env")
                ...
                .and_then(|v| v.as_str()),
            Some("claude-new"),
            "model override should follow the target provider verbatim"
        );
        let live_env = live
            .get("env")
        ...
        assert_eq!(
            live_env
                ...
                .and_then(|v| v.as_str()),
            Some("deepseek-v4-flash"),
            "Haiku role should keep the provider's real model name"
        );
        assert_eq!(
            live_env
        ...
        assert_eq!(
            ...
            "Sonnet role should keep the provider's 1M marker verbatim for Claude Code"
        );
        assert_eq!(
            live_env
        ...
        assert_eq!(
            ...
            "Opus role should keep the provider's model name and 1M marker verbatim"
        );
        assert!(
            live_env.get("ANTHROPIC_DEFAULT_OPUS_MODEL_NAME").is_none(),
            "display names are no longer synthesized when the provider does not configure them"
        );
