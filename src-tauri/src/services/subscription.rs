...
/// 查询 Codex / ChatGPT 订阅额度
///
/// `tool_label` 与 `expired_message` 由调用方传入（CLI 凭据路径使用
/// `"codex"` + "Please re-login with Codex CLI."）。
...
/// 桌面应用的 OAuth client 凭据本身不属于机密；用 concat! 拼接是为了避开
/// GitHub push protection 对该公开值的误报拦截，编译结果与直接书写完全一致。
