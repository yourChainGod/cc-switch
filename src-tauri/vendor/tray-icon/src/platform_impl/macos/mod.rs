...
impl TrayIcon {
    ...
    fn remove(&mut self) {
        ...
        {
            // PATCH(cc-switch): unregister tracking areas and detach the view
            // while the view→window link is still intact, BEFORE removing the
            // status item. NSTrackingArea does not retain its owner; with the
            // upstream order the status bar window keeps routing enter/exit
            // events to the freed TrayTarget (SIGABRT/SIGBUS on quit/restart
            // while the mouse hovers the tray).
            let areas = tray_target.trackingAreas();
            for area in areas {
                tray_target.removeTrackingArea(&area);
            }
        ...
        // PATCH(cc-switch): defer the TrayTarget release to the autorelease pool
        // so an enter/exit event already routed in the current runloop turn still
        // finds a live owner.
        if let Some(tray_target) = self.tray_target.take() {
            let _ = Retained::autorelease_return(tray_target);
        }
    }
...
fn send_mouse_event(
    ...
    // icon position & size
    // PATCH(cc-switch): the event's window can be nil while the status item
    // is being torn down; bail out instead of panicking inside an objc frame.
