...
- Claude / Codex **添加供应商时可一次粘贴多个 Key**（空格 / 逗号 / 换行分隔），创建后自动组成 Key 池；主配置只写入首个 Key，完整列表进入池内调度
- 直连配置使用最高优先级可用 Key，代理路由共享同一个池自动调度；支持在详情里查看本次请求实际使用的 Key

### 🔀 增强：通道级故障转移与会话亲和

- 重试 / 熔断 / 冷却从"供应商级"细化到 **`provider:key` 通道级**，单 Key 失效不会拖垮整个供应商（指数退避冷却）
- **同层随机调度**：优先级与连续失败次数仍是硬排序；同优先级、同健康度的 Key 每次请求随机打散，避免大量 Key 场景下固定前排通道反复撞 429
- **限流感知调度**：429 默认只降权不冷却；429/5xx 带上游 `Retry-After` 时精确冷却该秒数；全部 Key 冷却时 503 响应附带 `Retry-After` 退避提示
...
- **请求详情诊断**：用量统计可展开单条请求详情，查看成本明细、Token、延迟、使用 Key、路由决策链、每次尝试的供应商 / Key / 状态码；错误正文默认折叠，Key 默认脱敏并支持点击复制
