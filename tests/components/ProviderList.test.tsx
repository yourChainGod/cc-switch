...
describe("ProviderList Component", () => {
  ...
  it("passes provider key summaries to the matching provider card", async () => {
    const providerA = createProvider({ id: "a", name: "A" });
    const providerB = createProvider({ id: "b", name: "B" });
    const keySummary = {
      appType: "claude",
    ...
    server.use(
      http.post("http://tauri.local/get_provider_key_summaries", () =>
        HttpResponse.json([keySummary]),
      ),
    );
    ...
    await waitFor(() => {
      ...
      for (const [props] of providerCardRenderSpy.mock.calls) {
        latestByProvider.set(props.provider.id, props);
      }

      expect(latestByProvider.get("b")?.keySummary).toMatchObject(keySummary);
      expect(latestByProvider.get("a")?.keySummary).toBeUndefined();
    });
  });
